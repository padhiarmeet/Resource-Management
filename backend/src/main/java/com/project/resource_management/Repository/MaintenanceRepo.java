...
public interface MaintenanceRepo extends JpaRepository<Maintenance, Integer> {
    ...
    // Find all maintenance for resources in a specific building
    @Query("SELECT m FROM Maintenance m WHERE m.resource.building.building_id = :buildingId")
    List<Maintenance> findByBuildingId(@Param("buildingId") int buildingId);
}
